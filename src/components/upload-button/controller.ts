		...
		DocumentPicker.show(
			...
			(error, res) => {
				...
				RNFS.readFile(url).then(res => {
					...
					open({ schema: [WeatherProp.schema, WeatherHead.schema] }).then(
						realm => {
							try {
								const heads = realm
									.objects<WeatherHead>("WeatherHead")
									.filtered(`name == "${headers[0]}"`);
								if (heads.isEmpty()) {
									throw new Error();
								}
							} catch {
							...
							try {
								const proprieties = realm
									...
									.filtered(`day == "${fileName}"`);
								if (proprieties.isEmpty()) throw new Error();
							} catch {
